...
router.get("/users", async (req, res) => {
  ...
  res.status(200).json({ email: "ffff" });
});
...
router.post("/register", async (req, res) => {
  ...
  //   });
  // }
  //551a70c29d91bc8b4c59bed50e38cd53
  /**
...
// טיפוס למערך כולו
type CountryFlagsResponse = CountryFlag[];

let c:CountryFlagsResponse;



router.get("/flags", async (req, res) => {

  axios.get<CountryFlagsResponse>('https://restcountries.com/v3.1/all?fields=flags')
    .then(function (response) {
      return response.data;
    }).then(function (data) {
      
      const flagsArray = data.map((item) => {
        return item.flags;

      })
      res.status(200).json({ flags: flagsArray });
    })
